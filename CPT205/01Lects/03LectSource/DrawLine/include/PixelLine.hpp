...
struct Pixel2D {
    ...
    int y;
    Color4f color;
    Pixel2D(int x, int y, Color4f color)
        : x(x), y(y), color(color) {}
...
struct Pixel2DHash {
    std::size_t operator()(const Pixel2D& p) const {
        return std::hash<int>()(p.x) ^
               std::hash<int>()(p.y);  // Add color if needed
    }
};
...
void renderPixelSet(PixelSet& pixel_set) {
    ...
    for (auto& ele : pixel_set) {
        glColor4d(ele.color._r, ele.color._g, ele.color._b,
                  ele.color._a);
        // 以整数为像素“边界”
...
struct Line2D {
    Line2D(std::pair<double, double> p1,
           std::pair<double, double> p2, Color4f color)
        : _p1(p1), _p2(p2), _color(color) {}

    std::pair<double, double>
        _p1;  // the start point of the line
    ...
    [[nodiscard]] static inline int ifloor(double v) {
        return (int)std::floor(v);
    }
    [[nodiscard]] static inline double frac(double v) {
    ...
    void calcPixelDDA(PixelSet& pixel_set) {
        ...
        double tMaxX = (stepX > 0) ? (1.0 - frac(x0)) * invAbsDx
                                   : frac(x0) * invAbsDx;
        double tMaxY = stepY > 0 ? (1.0 - frac(y0)) * invAbsDy
                                 : frac(y0) * invAbsDy;
